...
scenarios('../features/list_active_templates.feature')
...
def check_active_templates(count):
    reset_db()
    insert_samples(count)
    cmd("src/stupa -j count_active")
    assert RC == 0
...
def run_active_json():
    cmd("src/stupa -j active")
    assert RC == 0


@when('stupa active is invoked successfully')
def run_active():
    cmd("src/stupa active")
    assert RC == 0
